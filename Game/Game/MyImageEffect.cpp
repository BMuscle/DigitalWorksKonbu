...
bool MyImageEffect::update(float t, const Vec2& pos) {//オーバライド　計算処理　描画処理も兼ねている
	if (t / switchingTime >= animationTexture.size()) {
		animationTexture.at(animationTexture.size() - 1).rotated(ToRadians(rotate)).drawAt(pos);//順番に描画する
		return false;//描画終了
	}
	animationTexture.at(t / switchingTime).rotated(ToRadians(rotate)).drawAt(pos);//順番に描画する
